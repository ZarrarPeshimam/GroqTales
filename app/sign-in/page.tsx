...
export default function SignInPage() {
  ...
  const handleSignIn = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Ensure proper identifier check for email layout
      const isEmail = /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(identifier);

      if (!isEmail) {
        // Assume username lookup via Supabase
  ...
  return (
    <div className="min-h-[calc(100vh-80px)] w-screen relative left-1/2 right-1/2 -ml-[50vw] -mr-[50vw] -mt-6 -mb-6 flex items-center justify-center bg-[#0a0a0c] text-neutral-200">
      
      {/* Abstract Corporate/CRM style background without excessive animations or emotes */}
      <div className="absolute inset-0 z-0 overflow-hidden pointer-events-none">
        <div className="absolute top-0 right-0 w-[40vw] h-[40vw] rounded-full bg-blue-900/10 blur-[120px]" />
        <div className="absolute bottom-0 left-0 w-[50vw] h-[50vw] rounded-full bg-indigo-900/10 blur-[120px]" />
      </div>
      ...
      <div className="w-full max-w-md px-6 py-12 relative z-10 flex flex-col">
        ...
        <p className="mt-8 text-center text-sm text-neutral-500 font-medium">
          ...
          <Link href="/sign-up" className="text-white hover:text-neutral-300 transition-colors ml-1 font-semibold">
            Register Here
          </Link>
        </p>
