-- ============================================================================
-- GroqTales Supabase Schema (Complete)
...
CREATE POLICY "Users can view their own drafts"
  ON drafts FOR SELECT USING (auth.uid() = owner_id);
CREATE POLICY "Users can create their own drafts"
  ON drafts FOR INSERT WITH CHECK (auth.uid() = owner_id);
CREATE POLICY "Users can update their own drafts"
  ON drafts FOR UPDATE USING (auth.uid() = owner_id);
CREATE POLICY "Users can delete their own drafts"
  ON drafts FOR DELETE USING (auth.uid() = owner_id);
...
BEGIN
  ...
  VALUES (
    ...
    COALESCE(NEW.raw_user_meta_data->>'firstName', NEW.raw_user_meta_data->>'first_name', 'Anonymous'),
    COALESCE(NEW.raw_user_meta_data->>'lastName', NEW.raw_user_meta_data->>'last_name', 'Creator'),
